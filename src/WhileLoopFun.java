public class WhileLoopFun {
	...
	public void printDigits(int number) {
		String number1 = new Integer(number).toString();
		int length = number1.length();
		int count = length-1;
		while (count != -1) {
			String digits = number1.substring(count, count + 1);
	...
	public int countLetter(String word, String letter) {
		...
		int count2 = 0;
		while (count != length) {
			String let = word.substring(count, count + 1);
			boolean equal = let.equals(letter);
			if (equal) {
				count2++;
			}
			count++;
